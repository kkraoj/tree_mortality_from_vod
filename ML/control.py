...
###### BEGIN YOUR CODE ######
# TODO:
V=np.random.rand(NUM_STATES)*0.1
P=np.zeros((NUM_STATES,2,NUM_STATES))+1/NUM_STATES
Rs=np.repeat(0.0,NUM_STATES)
...
# here, and say the overall algorithm has converged.
action_count=np.zeros((NUM_STATES,2,NUM_STATES))
...
#    action = 0 if np.random.uniform() < 0.5 else 1
    action = np.argmax(P[state].dot(V))
    if action ==1:
        n_action+=1
    ###### END YOUR CODE ######
...
#    raise NotImplementedError('Update T and R not implemented')
    ...
    action_count[state,action,new_state]+=1
...
#        if np.sum(action_count[state,action,:])>0 & \
#                 action_count[state,action,new_state]>1:
#            P[state, action, new_state]=action_count[state,action,new_state]/\
#                        action_count[state,action,:].sum()
#            P[state,action,:]/= P[state,action,:].sum()
        P=action_count.copy()
#        P[(P==0)]=1/NUM_STATES
#        P/=P.sum(2,keepdims=True)
        dr=P.sum(2,keepdims=True)
        cond=dr>1
        P=np.divide(P,dr,out=np.zeros_like(P)+1/NUM_STATES,where=cond)
        P/=P.sum(2,keepdims=True)
        ###### END YOUR CODE ######
...
#        raise NotImplementedError('Value iteration choice not implemented')
        ...
        while difference>TOLERANCE:                
            V_new=Rs+GAMMA*np.max(P.dot(V),axis=1)
            difference=max(np.abs(V_new-V))
            V=V_new.copy()
            niter+=1
#            print(difference)
#            if niter==20:
...
#                break    
    ...
    if new_state == NUM_STATES - 1:
        ...
        time_steps_to_failure.append(time - 0.)
        time_steps_to_failure[num_failures-1] = time - time_at_start_of_current_trial
...
######plot the learning curve (time balanced V. trial)
...
x_ind = np.arange(window//2, len(log_tstf) - window//2)
plt.plot(x_ind, weights[window:len(log_tstf)], 'r--')
plt.xlabel('Num failures')
#plt.yscale('log')
plt.ylabel('Log Num steps to failure')
plt.savefig('MDP.png')
plt.show()
